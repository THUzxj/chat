...
def publish_image(topic, name, ref, size=None):
    tid = next_id()
    return pb.ClientMsg(pub=pb.ClientPub(id=tid, topic=topic, head={"mime": json.dumps("text/x-drafty").encode('utf-8')},
                                         content=json.dumps(
        {
            "ent": [
                {
    ...
    ).encode("utf-8")), extra=pb.ClientExtra(attachments=[ref,]))

def publish_video(topic, name, ref, size=None):
    tid = next_id()
    return pb.ClientMsg(pub=pb.ClientPub(id=tid, topic=topic, head={"mime": json.dumps("text/x-drafty").encode('utf-8')},
                                         content=json.dumps(
        {
...
def publish_video_avi(topic, name, ref, size=None):
    tid = next_id()
    return pb.ClientMsg(pub=pb.ClientPub(id=tid, topic=topic, head={"mime": json.dumps("text/x-drafty").encode('utf-8')},
                                         content=json.dumps(
        {
...
def publish_file(topic, name, ref):
    tid = next_id()
    return pb.ClientMsg(pub=pb.ClientPub(id=tid, topic=topic, head={"mime": json.dumps("text/x-drafty").encode('utf-8')},
                                         content=json.dumps(
        {
